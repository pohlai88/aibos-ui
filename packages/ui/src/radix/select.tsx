/**
 * Select Wrapper - Enterprise Production Ready
 *
 * Thin wrapper over @radix-ui/react-select with semantic tokens.
 * Provides accessible select components with proper keyboard navigation.
 */
...
>(({ className, children, ...props }, reference) => (
  <SelectPrimitive.Trigger
    ref={reference}
    className={cn(
      'border-semantic-border bg-semantic-background ring-offset-semantic-background placeholder:text-semantic-muted-foreground focus:ring-semantic-ring flex h-10 w-full items-center justify-between rounded-md border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1',
      className,
    )}
    {...props}
  >
...
>(({ className, ...props }, reference) => (
  <SelectPrimitive.Label
    ref={reference}
    className={cn('text-semantic-foreground py-1.5 pl-8 pr-2 text-sm font-semibold', className)}
    {...props}
  />
));
