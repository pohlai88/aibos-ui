...
function validatePhase1() {
  ...
  results.infrastructure.push(
    checkFileExists('docker-compose.yml', 'Docker Compose configuration'),
  );
  ...
  results.quality.push(
    execCommand('pnpm dx', 'Development quality check (format, lint, typecheck, test, deps)'),
  );
  ...
  results.documentation.push(
    checkFileExists('docs/adr/README.md', 'Architecture Decision Records'),
  );
  results.documentation.push(checkFileExists('docs/development/README.md', 'Development guide'));
  results.documentation.push(
    checkFileExists('docs/training/anti-drift-guardrails.md', 'Anti-drift training'),
  );
  ...
  results.deployment.push(
    execCommand(
      'curl -f http://localhost:3000 || echo "Web app not running"',
      'Web application health',
    ),
  );
  results.deployment.push(
    execCommand(
      'curl -f http://localhost:3001/health || echo "BFF not running"',
      'BFF health check',
    ),
  );
