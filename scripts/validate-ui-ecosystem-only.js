...
class UIEcosystemValidator {
  ...
  async checkUILayerBoundaries() {
    ...
    try {
      ...
      const result = execSync(
        'npx dependency-cruiser --config .dependency-cruiser.js --output-type json packages/ui packages/ui-business apps/web apps/bff',
        {
          encoding: 'utf8',
          stdio: 'pipe',
        },
      ...
      if (violations.summary && violations.summary.violations) {
        ...
        if (uiViolations.length === 0) {
          ...
          this.log(
            `   ❌ ISSUES - Found ${uiViolations.length} UI Layer Boundary Violations`,
            'error',
          );
  ...
  isUIViolation(violation) {
    const uiPatterns = [/packages\/ui/, /packages\/ui-business/, /apps\/(web|bff).*ui/];
    ...
    return (
      uiRuleNames.includes(violation.rule.name) ||
      uiPatterns.some((pattern) => pattern.test(violation.from) || pattern.test(violation.to))
    );
  }
  ...
  async checkUIPolymorphicComponents() {
    ...
    if (fs.existsSync(safeJoin(BASE_DIR, primitivesDir))) {
      const files = fs.readdirSync(safeJoin(BASE_DIR, primitivesDir)).filter((f) => f.endsWith('.tsx'));
      files.forEach((file) => {
    ...
    if (fs.existsSync(safeJoin(BASE_DIR, componentsDir))) {
      const files = fs.readdirSync(safeJoin(BASE_DIR, componentsDir)).filter((f) => f.endsWith('.tsx'));
      files.forEach((file) => {
    ...
    if (fs.existsSync(safeJoin(BASE_DIR, hooksDir))) {
      const files = fs.readdirSync(safeJoin(BASE_DIR, hooksDir)).filter((f) => f.endsWith('.tsx'));
      files.forEach((file) => {
    ...
    try {
      ...
      if (fs.existsSync(safeJoin(BASE_DIR, uiPackagePath))) {
        ...
        const businessDeps = Object.keys(uiPackage.dependencies || {}).filter(
          (dep) =>
            dep.includes('accounting') ||
            dep.includes('eventsourcing') ||
            dep.includes('observability') ||
            dep.includes('policy'),
        );
        ...
        if (businessDeps.length === 0) {
          ...
          this.log(
            `   ❌ ISSUES - UI Package has business dependencies: ${businessDeps.join(', ')}`,
            'error',
          );
          this.summary.uiIssues++;
          this.summary.errors++;
  ...
  async checkUIBuildSystem() {
    ...
    try {
      ...
      if (fs.existsSync(safeJoin(BASE_DIR, 'packages/ui-business/package.json'))) {
  ...
  async checkGeneralDependencies() {
    ...
    try {
      ...
      const result = execSync(
        'npx dependency-cruiser --config .dependency-cruiser.js --output-type json',
        {
          encoding: 'utf8',
          stdio: 'pipe',
        },
      ...
      if (violations.summary && violations.summary.violations) {
        ...
        if (generalViolations.length === 0) {
          ...
          this.log(
            `   ⚠️ ISSUES - Found ${generalViolations.length} General Dependency Violations`,
            'warning',
          );
  ...
  generateComprehensiveReport() {
    ...
    if (this.summary.uiIssues === 0) {
      ...
      this.log('║                                                                          ║');
      this.log(
        `║  🔧 Found ${this.summary.uiIssues} UI ecosystem issues that need attention              ║`,
      );
      this.log('║                                                                          ║');
      this.log(
        `║     ❌ Errors:   ${this.summary.errors.toString().padEnd(3)}                                                      ║`,
      );
      this.log(
        `║     ⚠️  Warnings: ${this.summary.warnings.toString().padEnd(3)}                                                      ║`,
      );
      this.log('║                                                                          ║');
    ...
    if (this.summary.nonUiIssues === 0) {
      ...
      this.log('║                                                                          ║');
      this.log(
        `║  🔧 Found ${this.summary.nonUiIssues} general dependency violations                    ║`,
      );
      this.log('║                                                                          ║');
    ...
    if (totalIssues === 0) {
      ...
      this.log(
        `║  📋 Total Issues Found: ${totalIssues.toString().padEnd(3)}                                                    ║`,
      );
      this.log('║                                                                          ║');
      this.log(
        `║     🎨 UI Ecosystem Issues:     ${this.summary.uiIssues.toString().padEnd(3)}                                      ║`,
      );
      this.log(
        `║     🔍 General Dependency Issues: ${this.summary.nonUiIssues.toString().padEnd(3)}                                      ║`,
      );
      this.log('║                                                                          ║');
